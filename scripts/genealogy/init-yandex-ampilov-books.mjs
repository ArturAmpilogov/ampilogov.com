...
const outputPath = path.join(root, "data/genealogy/searches/yandex-archive-ampilov-books-2026-09-10.json");
...
function collectUrls(value, urls = new Set()) {
  if (typeof value === "string") { const url = canonicalYandexUrl(value); if (url) urls.add(url); }
...
function extractNextData(html) {
  ...
  if (start < 0 || end < 0) throw new Error("В HTML нет __NEXT_DATA__");
  return JSON.parse(html.slice(start + marker.length, end));
}
function displayDate(from, to) {
  const months = ["января","февраля","марта","апреля","мая","июня","июля","августа","сентября","октября","ноября","декабря"];
  const format = (value) => { const [d,m,y] = value.split("-").map(Number); return `${d} ${months[m-1]} ${y}`; };
  if (!from) return "дата не указана";
  if (!to || to === from) return format(from);
...
for (const file of await filesRecursive(sourcesRoot, ".json")) {
  ...
  for (const url of collectUrls(source)) {
    ...
    linked.push({sourceId:source.sourceId, reviewStatus:source.review?.status ?? ""});
...
for (let page = 1; page <= 4; page++) {
  const html = await readFile(`/private/tmp/ampilov-books-page${page}.html`, "utf8");
  const props = extractNextData(html).props.pageProps;
  if (Number(props.pageNum) !== page || Number(props.totalPages) !== 4 || Number(props.totalDocs) !== 32) throw new Error(`Неверная страница ${page}`);
  for (let position = 0; position < props.items.length; position++) {
    ...
    const directory = path.join(evidenceRoot, item.parentId);
    const evidence = await filesRecursive(directory);
    const completeEvidence = [
      `${number}-full-view.png`, `${number}-header.png`, `${number}-target-entry.png`,
    ...
    results.push({
      absolutePosition: results.length + 1,
      page,
      position: position + 1,
      catalogId: item.parentId,
      scanNumber: item.sheetPageNumber,
      documentUrl,
      title: breadcrumbs.at(-1)?.name ?? item.name,
      publication: breadcrumbs.map((entry) => entry.name).join(", "),
      date: displayDate(item.dateFrom, item.dateTo),
      indexSnippet: (item.snippet ?? "").replace(/[\u0000-\u001f\u007f]/g, "").replace(/[\[\]]/g, "").trim(),
      status,
      capture: status !== "existing-complete",
      sourceIds: linked.length ? [...new Set(linked.map((entry) => entry.sourceId))].sort() : undefined,
    });
  }
}
...
const manifest = {
  schemaVersion:1,
  searchRunId:"yandex-archive-ampilov-books-2026-09-10",
  status:"inventory-complete-processing-in-progress",
  createdAt:"2026-09-10",
  queryText:"Ампилов",
  rules:{sort:"ascending-by-date", exactQuery:true, deduplicateByCatalogAndScan:true, verifyAgainstPrimaryScan:true, localEvidenceRequired:true, publicYandexLinkOnly:true, publicCutoffYear:1950},
  progress:{pagesInventoried:4, rowsFound:32, rowsCompleted:results.length-pending.length, uniqueScansPending:new Set(pending.map((row)=>`${row.catalogId}/${row.scanNumber}`)).size, rowsRemaining:pending.length},
  batches:[{index:"books", searchUrl:"https://yandex.ru/archive/search?text=%D0%90%D0%BC%D0%BF%D0%B8%D0%BB%D0%BE%D0%B2&index=books&updateDate=0&excludeSeen=0&rankMode=by_date&sortOrder=ascending", reportedPages:4, reportedResults:32, results}],
};
